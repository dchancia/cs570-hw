...
for idx in range(5):
    ...
    fo = open(os.path.join(".", "closed", "closed-" + str(idx) + ".txt"), "w")
    ...
    for i in range(len(list_terms)):
        for j in range(len(list_terms)):
            if i != j:
                if (set(list_terms[i]).issubset(set(list_terms[j])) and list_support[i] == list_support[j]):
                    non_closed.append((list_support[i], list_terms[i]))

    for elems in non_closed:
        if elems not in non_closed_unique:
            non_closed_unique.append(elems)

    for item in non_closed_unique:
